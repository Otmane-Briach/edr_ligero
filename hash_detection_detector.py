...
class Event:
    ...
    # NUEVO: Campo para URL en eventos EXEC de wget/curl
...
class ProcessScoreTracker:
    ...
    def __init__(self, detector=None):  
        ...
        self.mitre_mapping = {
            ...
            'rapid_fork': ['T1106', 'T1543'],  # Native API, Create System Process
            'download_execute_chain': ['T1105', 'T1059'],  # Ingress Tool Transfer + Command Execution
            'multi_arch_payload': ['T1608.001'],  # Stage Capabilities
            'directory_scan': ['T1083'],  # File and Directory Discovery
            'recursive_scan': ['T1083'],  # File and Directory Discovery  
            'chmod_burst': ['T1222.002']  # File Permissions Modification
        ...
        if points > 0:
            ...
            if hasattr(self.detector, 'process_tree') and self.detector.process_tree.tree:
                ...
                if ppid:
                    ...
                    family_score = 0
                    all_indicators = family['indicators'].copy()  # Copiar indicadores de hijos

                    # NUEVO: Incluir indicadores del proceso padre si existen
                    if ppid in self.process_scores:
                        parent_events = self.process_scores[ppid]['window_events']
                        for e in parent_events:
                            indicator_name = e[1]
                            all_indicators[indicator_name] = all_indicators.get(indicator_name, 0) + 1

                    unique_indicators = set(all_indicators.keys())
                    for ind in unique_indicators:
                        base_points = self.scoring_rules.get(ind, 0)
                        family_score += base_points  # Solo cuenta UNA vez cada tipo
                        
                    # Bonus por combinaciones específicas de ransomware
                    if {'locked_files_burst', 'unlink_burst'}.issubset(unique_indicators):
                        family_score += 2  # Cifrar + borrar = ransomware clásico
                        
                    if {'massive_write', 'unlink_burst'}.issubset(unique_indicators):
                        family_score += 1  # Escribir + borrar = sospechoso

                    # Penalización si es SOLO un tipo repetido (reduce FP)
                    if len(unique_indicators) == 1 and 'massive_write' in unique_indicators:
                        family_score = min(family_score, 3)  # Cap a 3 puntos si es solo writes
                    ...
                    if family_score >= self.alert_threshold and family_id not in self.alerted_families:
                        ...
                        # metrica
                        if self.detector and hasattr(self.detector, 'timing_metrics'):
                            if not self.detector.timing_metrics['ransomware_detection_time']:
                                self.detector.timing_metrics['ransomware_detection_time'] = event_time
                                time_to_detect = event_time - self.detector.timing_metrics['first_event_time']
                                print(f"METRICS::RANSOMWARE_DETECTED::{time_to_detect:.3f}::{self.detector.timing_metrics['events_processed_until_alert']}", file=sys.stderr)

                        indicators_str = ', '.join([f"{ind}(x{cnt})" for ind, cnt in all_indicators.items()])
                        mitre_techniques = set()
                        for ind in all_indicators.keys():
            ...
            if verbose or current_score >= 4:
                time_elapsed = event_time - self.detector.timing_metrics['first_event_time'] if self.detector else 0
                print(f"SCORING PID {pid}: {current_score:.1f}/{self.alert_threshold} pts @ {time_elapsed:.3f}s [decay: {', '.join(indicators_with_decay[:5])}]", file=sys.stderr)
            ...
            if current_score >= self.alert_threshold and pid not in self.alerted_pids:
                ...
                # MÉTRICA: Tiempo hasta detección de ransomware
                if self.detector and hasattr(self.detector, 'timing_metrics'):
                    if not self.detector.timing_metrics['ransomware_detection_time']:
                        self.detector.timing_metrics['ransomware_detection_time'] = event_time
                        time_to_detect = event_time - self.detector.timing_metrics['first_event_time']
...
class ThreatDetectorFixed:
    ...
    def __init__(self):
        ...
        self.double_ext_windows = defaultdict(deque)
        self.read_write_tracking = defaultdict(lambda: {'reads': set(), 'writes': set(), 'last_seen': 0})  # AÑADIR ESTA LÍNEA
        ...
        self.config = {
            ...
            "suspicious_extensions": {
                ...
                ".vault", ".crypto", ".secure", ".ransomed", ".sougolock", ".Ra_Lock"   # <-- AÑADIR
            },
        ...
        self.timing_metrics = {
            ...
            'first_suspicious_event_time': None,
            'first_alert_time': None,
            'time_to_detection': None,
            'events_processed_until_alert': 0,
            'alert_timeline': [],  # [(timestamp, alert_type, score, pid)]
            'ransomware_detection_time': None,
            'indicators_timeline': []  # [(timestamp, indicator, pid)]
        }
    ...
    def analyze_event(self, event_data: dict) -> Optional[str]:
        ...
        try:
            ...
            # MÉTRICA: Registrar primer evento
            if not self.timing_metrics['first_event_time']:
                self.timing_metrics['first_event_time'] = event.timestamp
                print(f"METRICS::FIRST_EVENT::{event.timestamp}", file=sys.stderr)
            ...
            if event.url and event.event_type == "EXEC":
                ...
                if event.path:
                    exe_name = event.path.split('/')[-1]
                    if exe_name in ['wget', 'curl']:
                        event.comm = exe_name  # Sobrescribir comm con el ejecutable real
                        print(f"DEBUG UPDATE: Changed comm to {event.comm}, url={event.url}", file=sys.stderr)

            # DEBUG: Verificar que el Event tiene la información correcta
            if event.event_type == "EXEC" and event.comm in ['wget', 'curl']:
                print(f"DEBUG EVENT READY: comm={event.comm}, url={event.url}, will check Mirai", file=sys.stderr)
            ...
            if alert_message:
                ...
                # MÉTRICA: Registrar alerta en timeline
                self.timing_metrics['alert_timeline'].append(
                    (event.timestamp, alert_level, alert_message[:50], event.pid)
                )
                
                if not self.timing_metrics['first_alert_time']:
                    self.timing_metrics['first_alert_time'] = event.timestamp
                    detection_latency = event.timestamp - self.timing_metrics['first_event_time']
                    print(f"METRICS::FIRST_ALERT::{detection_latency:.3f}::events={self.timing_metrics['events_processed_until_alert']}", file=sys.stderr)
    ...
    def _check_write_burst(self, pid: int) -> Optional[str]:
        ...
        if ops >= 50 or bytes_total >= 20*1024*1024:  # Umbrales según tu doc
            ...
            # MÉTRICA: Registrar massive_write
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'massive_write', pid)
            )
            if not self.timing_metrics['first_suspicious_event_time']:
                self.timing_metrics['first_suspicious_event_time'] = current_time
                print(f"METRICS::FIRST_SUSPICIOUS::{event.timestamp - self.timing_metrics['first_event_time']:.3f}", file=sys.stderr)
            ...
            if event.mode and (event.mode & 0o111):  # Permisos de ejecución
                ...
                if event.ppid in self.process_tree.tree:
                    ...
                    if parent.get('exe') in ['wget', 'curl']:
                        # MÉTRICA: Download-execute
                        self.timing_metrics['indicators_timeline'].append(
                            (event.timestamp, 'download_execute_chain', event.pid)
                        )
        ...
        if event.event_type == "EXEC":
            ...
            if len(recent) >= 5:  # 5+ forks en 5s
                ...
                if parent.get('exe') not in ['make', 'gcc', 'g++', 'cargo', 'npm', 'python']:
                    # MÉTRICA: Rapid fork
                    self.timing_metrics['indicators_timeline'].append(
                        (event.timestamp, 'rapid_fork', event.ppid)
                    )
    ...
    def _check_file_burst_antispam(self, event: Event) -> Optional[str]:
        ...
        if len(window) >= self.config["file_burst_threshold"]:
            # MÉTRICA: Registrar indicador en timeline
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'locked_files_burst', pid)
            )
            if not self.timing_metrics['first_suspicious_event_time']:
                self.timing_metrics['first_suspicious_event_time'] = current_time
                print(f"METRICS::FIRST_SUSPICIOUS::{event.timestamp - self.timing_metrics['first_event_time']:.3f}", file=sys.stderr)
    ...
    def _check_double_extension_pattern(self, event: Event) -> Optional[str]:
        ...
        # Extensiones legítimas de backup/temp que ignoramos
        whitelist_second_ext = r'\.(bak|backup|tmp|temp|old|orig|copy|\d{8}|\d{6})$'
        ...
        if re.search(double_ext_pattern, event.path.lower()):
            # CLAVE: No alertar por UN archivo, sino por MÚLTIPLES
            pid = event.pid
            current_time = event.timestamp
            
            # Tracking de archivos con doble extensión por PID
            if not hasattr(self, 'double_ext_windows'):
                self.double_ext_windows = defaultdict(deque)
                self.read_write_tracking = defaultdict(lambda: {'reads': set(), 'writes': set(), 'last_seen': 0})
            
            window = self.double_ext_windows[pid]
            # Limpiar ventana de 10 segundos
            while window and current_time - window[0][0] > 10:
                window.popleft()
            
            # Añadir este archivo
            window.append((current_time, event.path))
            
            # Solo alertar si vemos MÚLTIPLES archivos con doble extensión
            if len(window) >= 3:  # 3+ archivos en 10 segundos
                # Verificar que no sean la misma extensión (podría ser legítimo)
                extensions = [path.split('.')[-1] for _, path in window]
                if len(set(extensions)) == 1:  # Misma extensión = probablemente ransomware
                    # MÉTRICA: Doble extensión
                    self.timing_metrics['indicators_timeline'].append(
                        (current_time, 'double_extension', pid)
                    )
                    alert = self.score_tracker.add_indicator(
                        pid, 'locked_files_burst', current_time
                    )
                    if alert:
                        window.clear()  # Anti-spam
                        return alert
        
        return None
    def _check_read_write_pattern(self, event: Event) -> Optional[str]:
        """Detectar patrón read-write típico de ransomware"""
        if event.event_type != "OPEN":
            return None
        ...
        critical_extensions = ('.vmdk', '.vmem', '.pdf', '.docx', '.xlsx')
        if not any(event.path.endswith(ext) for ext in critical_extensions):
            return None
        
        pid = event.pid
        current_time = event.timestamp
        tracker = self.read_write_tracking[pid]
        
        # Limpiar datos antiguos (>30 segundos)
        ...
        if event.flags:
            if event.flags & 0x2:  # O_RDWR o O_WRONLY
                tracker['writes'].add(event.path)
            else:  # O_RDONLY
                tracker['reads'].add(event.path)
        
        # Detectar patrón: mismo archivo leído y luego escrito
        files_read_then_written = tracker['reads'] & tracker['writes']
        
        if len(files_read_then_written) >= 5:  # 5+ archivos con patrón read-write
            # MÉTRICA: Read-write pattern
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'read_write_pattern', pid)
            )


            alert = self.score_tracker.add_indicator(
                pid, 'massive_write', current_time, verbose=self.verbose_scoring
            )
            if alert:
                tracker['reads'].clear()
                tracker['writes'].clear()
    ...
    def _check_ransomware_behavior(self, event: Event) -> Optional[str]:
        """Detectar comportamiento de ransomware combinando señales"""
        
        if not event.path:
            return None
        
        # DEFINIR el patrón aquí también
        double_ext_pattern = r'\.(txt|doc|pdf|jpg|jpeg|png|csv|sql|db|xls|xlsx|ppt|pptx)\.[a-z0-9]{1,10}$'
        
        # Señal 1: Doble extensión
        has_double_ext = bool(re.search(double_ext_pattern, event.path.lower()))  # Añadir .lower()
        
        # Señal 2: El proceso también está BORRANDO archivos originales
        if event.event_type == "UNLINK" and event.pid in self.double_ext_windows:
            # Este PID creó archivos con doble extensión Y ahora borra archivos
            # MÉTRICA: Comportamiento ransomware
            self.timing_metrics['indicators_timeline'].append(
                (event.timestamp, 'ransomware_behavior', event.pid)
            )
        ...
        return None  # Añadir return None al final
    ...
    def _check_suspicious_locations_improved(self, event: Event) -> Optional[str]:
        ...
        for suspicious_path in self.config["suspicious_paths"]:
            if event.path.startswith(suspicious_path):
                ...
                if event.event_type == "EXEC":
                     # MÉTRICA: Ejecución desde /tmp
                    self.timing_metrics['indicators_timeline'].append(
                        (event.timestamp, 'tmp_execution', event.pid)
                    )
                    alert = self.score_tracker.add_indicator(
                ...
                if event.flags and event.flags & 0x200:
                    # MÉTRICA: O_TRUNC flag
                    self.timing_metrics['indicators_timeline'].append(
                        (event.timestamp, 'o_trunc', event.pid)
                    )
                    alert = self.score_tracker.add_indicator(
    ...
    def _check_mass_deletion(self, event: Event) -> Optional[str]:
        ...
        if len(window) >= 5:  # >5 en 10s según tu documento
            # MÉTRICA: Borrado masivo
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'unlink_burst', pid)
            )
            if not self.timing_metrics['first_suspicious_event_time']:
                self.timing_metrics['first_suspicious_event_time'] = current_time
                print(f"METRICS::FIRST_SUSPICIOUS::{event.timestamp - self.timing_metrics['first_event_time']:.3f}", file=sys.stderr)
    ...
    def _check_directory_scan(self, event: Event) -> Optional[str]:
        ...
        if len(window) >= 100:
            # MÉTRICA: Directory scan
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'directory_scan', pid)
            )
            alert = self.score_tracker.add_indicator(
    ...
    def _check_privilege_escalation(self, event: Event) -> Optional[str]:
        ...
        if "SETUID" in suspicious_perms or "SETGID" in suspicious_perms:
            ...
            # MÉTRICA: SETUID/SETGID
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'setuid_chmod', pid)
            )
            alert = self.score_tracker.add_indicator(pid, 'setuid_chmod', current_time, verbose=self.verbose_scoring)
        else:
            # MÉTRICA: chmod regular
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'chmod_regular', pid)
            )
    ...
    def _check_multi_arch_files(self, event: Event) -> Optional[str]:
        ...
        if len(self.arch_files_by_base[ppid][base_name]) >= 3:
            # MÉTRICA: Multi-arquitectura
            self.timing_metrics['indicators_timeline'].append(
                (event.timestamp, 'multi_arch_payload', ppid)
            )
            alert = self.score_tracker.add_indicator(
    ...
    def _check_mirai_download_pattern(self, event: Event) -> Optional[str]:
        ...
        # DEBUG
        if event.event_type == "EXEC":
            url_value = getattr(event, 'url', 'NO_ATTR')
            print(f"DEBUG MIRAI CHECK: comm={event.comm}, url='{url_value}', has_url={hasattr(event, 'url')}", file=sys.stderr)

            #AÑADIR: Debug específico para wget/curl
            if event.comm in ['wget', 'curl']:
                print(f"★ WGET/CURL DETECTED: {event.comm} {url_value}", file=sys.stderr)
        ...
        # AÑADIR DEBUG
        print(f"★ PROCESSING MIRAI: {event.comm}", file=sys.stderr)
        
        # USAR EL CAMPO URL DEL EVENTO
        url = getattr(event, 'url', None)
        if not url:
            print(f"★ NO URL for {event.comm}", file=sys.stderr)
            return None
        ...
        match = re.search(arch_pattern, url.lower())  # Buscar en URL, no en cmdline
        ...
        if len(tracker['architectures']) >= 3:
            # MÉTRICA: Mirai botnet pattern
            self.timing_metrics['indicators_timeline'].append(
                (current_time, 'botnet_multiarch_download', ppid)
            )
            if not self.timing_metrics['first_suspicious_event_time']:
                self.timing_metrics['first_suspicious_event_time'] = current_time
                print(f"METRICS::FIRST_SUSPICIOUS::{event.timestamp - self.timing_metrics['first_event_time']:.3f}", file=sys.stderr)
    ...
    def _check_process_injection(self, event: Event) -> Optional[str]:
        ...
        # MÉTRICA: ptrace
        self.timing_metrics['indicators_timeline'].append(
            (event.timestamp, 'ptrace_use', event.pid)
        )
        # ptrace → +1 punto
        alert = self.score_tracker.add_indicator(
    ...
    def _check_memory_execution(self, event: Event) -> Optional[str]:
        ...
        if event.suspicious_mmap:
            ...
            # MÉTRICA: mmap executable
            self.timing_metrics['indicators_timeline'].append(
                (event.timestamp, 'mmap_exec', event.pid)
            )
            alert = self.score_tracker.add_indicator(
    ...
    def _check_persistence_attempt(self, event: Event) -> Optional[str]:
        ...
        if persistence_type:
            ...
            # Añadir al scoring
            # MÉTRICA: Persistencia
            self.timing_metrics['indicators_timeline'].append(
                (current_time, persistence_type, pid)
            )
    ...
    def print_final_stats(self):
        ...
        if self.timing_metrics['ransomware_detection_time']:
            ...
            print(f"   Eventos procesados hasta alerta: {self.timing_metrics['events_processed_until_alert']}", file=sys.stderr)
        
        if self.timing_metrics['first_suspicious_event_time']:
            time_to_suspicious = self.timing_metrics['first_suspicious_event_time'] - self.timing_metrics['first_event_time']
            print(f"   Tiempo hasta primer evento sospechoso: {time_to_suspicious:.3f}s", file=sys.stderr)
        
        print(f"   Total alertas en timeline: {len(self.timing_metrics['alert_timeline'])}", file=sys.stderr)
        print(f"   Total indicadores detectados: {len(self.timing_metrics['indicators_timeline'])}", file=sys.stderr)
        
        # Exportar timeline a JSON para análisis
        import json
        with open('/tmp/detection_metrics.json', 'w') as f:
            json.dump(self.timing_metrics, f, indent=2)
        print(f"   Métricas exportadas a /tmp/detection_metrics.json", file=sys.stderr)
